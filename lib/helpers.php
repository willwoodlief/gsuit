...
function save_spreadsheet($inputFileName,$original_name) {
	...
	foreach ($data as $row) {
		...
		$insert = [
			...
			'website'=> $row['website'],
			'twitter'=> $row['twitter'],
			'facebook'=> $row['linkedin'],
			'linkedin'=> $row['facebook']
		];
...
function generate_footer($data) {
	global $spead_email,$spread_name,$spread_company,$spread_function,$spread_mobile,$spread_phone,
	       $spread_address_line1,$spread_address_line2,$spread_address_city,$spread_address_postal,$spread_website,
	       $spread_twitter,$spread_linkedin,$spread_facebook;
	...
	//first name	last name	company name	function	email	mobile	tel office
	//	address line 1	address line2	postal code	city	website	twitter	linkedin	facebook
	ob_start();
